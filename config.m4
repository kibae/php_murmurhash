...
dnl PHP_ARG_ENABLE(murmurhash, whether to enable murmurhash support,
dnl Make sure that the comment is aligned:
dnl [  --enable-murmurhash           Enable murmurhash support])

PHP_ARG_ENABLE(murmurhash, whether to enable murmurhash support, [  --enable-murmurhash     Enable murmurhash support])
...
if test "$PHP_MURMURHASH" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $MURMURHASH_DIR/lib, MURMURHASH_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_MURMURHASHLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong murmurhash lib version or lib not found])
  dnl ],[
  dnl   -L$MURMURHASH_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(MURMURHASH_SHARED_LIBADD)

  PHP_NEW_EXTENSION(murmurhash, murmurhash.c, $ext_shared)
fi
